...
3. **Held checks** — `rules/semgrep/securable.yaml` maps the skills' anti-pattern tags to enforceable semgrep rules (SSEM/ASVS metadata on each), tested against paired fail/pass fixtures; and `scripts/securability_report.sh` + `.github/workflows/securability-report.yml` produce the FIASSE S5.2.1 Securability Report on pull requests via any agent CLI (`claude`, `codex`, `opencode`) — advisory by default, per S5.2.2.
